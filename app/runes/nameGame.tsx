...
export default function RuneNameGame() {
  ...
  const [currentRune, setCurrentRune] = useState(remainingRunes[Math.floor(Math.random() * remainingRunes.length)]);
  ...
  function handleSelect(option: string) {
    ...
    if (option === currentRune.name) {
      ...
      setResult(`Try again. The correct answer is: ${currentRune.name}`);
    }
  }
  ...
  return (
    <div style={{
      ...
      <h2 style={{ color: "var(--primary)", fontWeight: 700, fontSize: "2rem", marginBottom: "1rem" }}>Rune Name Game</h2>
      <div style={{ fontWeight: 600, fontSize: "1.1rem", marginBottom: "1rem", color: "var(--primary)" }}>Score: {score} / {runes.length}</div>
      {!completed ? (
        <>
          <Image src={`/${currentRune.letter} - ${currentRune.name}.png`} alt={currentRune.letter} width={96} height={96} style={{ marginBottom: "1.5rem", borderRadius: "0.5rem", background: "var(--card)" }} />
